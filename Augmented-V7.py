...
def simplify_datainput(tablehead, dataframe):
    ...
    for i in range(0, shape[1]):
        if (transpose[i] == np.zeros((shape[0],),dtype=int)).all() or  ((transpose[i]-transpose[i][0]) == np.zeros((shape[0],), dtype=int)).all():
...
def match(rule, is_connected_closed, relations, relation_dic, attributes=None,
          attribite_dic=None):  # ,attribute_mapping_dic=None):
    # rule head
