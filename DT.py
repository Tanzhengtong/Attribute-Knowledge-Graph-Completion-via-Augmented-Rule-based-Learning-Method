...
if __name__ == '__main__':
    ...
    data_set,sc_min,sc_max,head_coverage,support_threshold,learning_time=\
        paramters[0],float(paramters[1]),float(paramters[2]),float(paramters[3]),int(paramters[4]),str(paramters[5])
    ...
    for i in range(0,len(rules)):
        pool.apply_async(augmented_rules, args=(i,relations,relation_dic,rules,sc_min,sc_max,head_coverage,support_threshold), callback=collect_result)
