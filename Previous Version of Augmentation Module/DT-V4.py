"""
V4：Support Attribute Knowledge Graph
"""
...
def categorical_to_discrete(path):
    ...
    # print(attribite_mapping_dic["common.topic.notable_types"])
...
def get_extra_attribute(bodys, attributes, attribite_dic,attribute_mapping_dic):
    ...
    for i in range(0, len(entities)):
        entity=entities[i]
        attribute_list.extend([attribute_mapping_dic.get(attribute).get(attribite_dic.get(entity + attribute,"None")) for attribute in attributes])
...
def simplify_datainput(tablehead, dataframe):
    ...
    for i in range(0, shape[1]):
        # print(transpose[i])
        # print(np.zeros((shape[0],), dtype=int))
        # print(np.ravel(np.full([1,shape[0]], np.nan)))
        # print((transpose[i] == np.zeros((shape[0],), dtype=int)).all() )
        # print((transpose[i] ==np.ravel(np.full([1,shape[0]], np.nan))).all())
        if (transpose[i] == np.zeros((shape[0],), dtype=int)).all():
            # or (transpose[i] == np.ravel(np.full([1, shape[0]], np.nan))).all():
...
def match(rule, relations, relation_dic,attributes=None, attribite_dic=None, attribute_mapping_dic=None):
    ...
    for result in results:
        ...
        if attribite_dic is not None and attributes is not None:
            extra_feature,extra_tablehead=get_extra_attribute(bodys, attributes, attribite_dic,attribute_mapping_dic)
...
def augmented_rules(i, relations, relation_dic, rules, sc_min, sc_max, head_coverage_threshold, support_threshold, attributes=None, attribite_dic=None, attribute_mapping_dic=None):
    cur_rule = re.sub(r'\t|\n|<--|\(|\)|,', ' ', rules[i]).split()
    ...
    # Data preparation
    X, Y, numerator, denominator, attributehead, headmatching = match(cur_rule, relations, relation_dic, attributes, attribite_dic,attribute_mapping_dic)
    # DT construction requirement:
    # support is at least greater than 10 and
    # SC cannot be 1
    print("No. %d" % i, numerator, denominator, np.array(X).shape)
...
if __name__ == '__main__':
    ...
    for i in range(0,len(rules)):
        pool.apply_async(augmented_rules,
                         args=(i, relations, relation_dic, rules, sc_min, sc_max, head_coverage, support_threshold,attributes, attribite_dic,attribute_mapping_dic),
    ...
    # T1 = time.time()
    # for i in range(0，len(rules)):
    #     augmented_rules(i, relations, relation_dic, rules, sc_min, sc_max, head_coverage, support_threshold,attributes, attribite_dic,attribute_mapping_dic)
    # T2 = time.time()
