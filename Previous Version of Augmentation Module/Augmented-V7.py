"""
V7: Attribute modification & Features simplification
"""
...
# def categorical_encoding(features):


def match(rule, is_connected_closed, relations, relation_dic, attributes=None,
          attribite_dic=None, contains_numerical_attribute=0):  # ,attribute_mapping_dic=None):
    # rule head
...
def augmented_rules(i, relations, relation_dic, rules, sc_min, sc_max, head_coverage_threshold, support_threshold,
                    ...
                    contains_numerical_attribute=0):  # , attribute_mapping_dic=None):
    cur_rule = re.sub(r'\t|\n|<--|\(|\)|,', ' ', rules[i]).split()
    ...
    with open('rules/' + data_set + '/augmented_rule-valid-test', 'a') as f:
        for r in result:
            f.write("\t".join(r) + '\r\n')
...
if __name__ == '__main__':
    ...
    for i in range(0, len(rules)):
        pool.apply_async(augmented_rules,
                         args=(
                         i, relations, relation_dic, rules, sc_min, sc_max, head_coverage, support_threshold, data_set,
    ...
    #                     attribite_dic,contains_numerical_attribute)
    # T2 = time.time()
