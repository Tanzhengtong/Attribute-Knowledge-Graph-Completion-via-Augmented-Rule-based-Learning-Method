...
def get_attribute_list(bodys):
    ...
    for entity in entities:
        attribute_list.extend([attribite_dic.get(entity + i, "None") for i in attributes])
...
def get_attribute(pair, relations, relation_dic):
    ...
    subj = pair[0]
    obj = pair[2]
    # find the attributes in which relation(subj,xx)
    subj_match = [find_attribute(subj, i, True, relation_dic) for i in relations]
    # find the attributes in which relation(xx,obj)
...
def augmented_rules(i,relations,relation_dic,rules,sc_min,sc_max,head_coverage_threshold,support_threshold):
    ...
    # Data preparation
    X, Y, numerator, denominator, attributehead, headmatching = match(rule, relations, relation_dic)
    ...
    if head_coverage < head_coverage_threshold:
        print("No. %d" % i, " Rule does not meet head coverage")
        return 0
    ...
    print("current confidence score: ",numerator / denominator,(tp)/(tp+fp), numerator / denominator > tp / (tp + fp))
...
if __name__ == '__main__':
    ...
    for i in range(0,len(rules)):
        pool.apply_async(augmented_rules,
                         args=(
                             i, relations, relation_dic, rules, sc_min, sc_max,
                             head_coverage,
                             support_threshold),
